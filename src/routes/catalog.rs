...
#[derive(Deserialize)]
pub struct InstanceUpdateInput {
    name: String,
...
pub async fn update_instance(
    ...
    u: CurrentUser,
    Path(id): Path<Uuid>,
    Json(i): Json<InstanceUpdateInput>,
) -> ApiResult<Json<InstanceView>> {
    u.require_resource_maintainer()?;
    if i.name.trim().is_empty() || i.name.chars().count() > 160 {
        return Err(ApiError::bad_request(
            "INVALID_NAME",
            "实例名称不能为空且最多 160 字符",
        ));
    ...
    if affected.rows_affected() == 0 {
        return Err(ApiError::conflict("组件实例已被修改"));
    }
    audit::record(
        &s.db,
        Some(u.id),

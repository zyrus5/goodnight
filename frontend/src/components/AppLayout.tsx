...
export function AppLayout(){
  ...
  return <div className="app-shell">
    ...
    <main className="workspace"><header className="topbar"><div><span className="crumb">统一发布控制台</span><strong>{currentNavigation?.[1]??'平台管理'}</strong></div><div className="topbar-user"><div className="avatar">{user.display_name.slice(0,1)}</div><div><strong>{user.display_name}</strong><small>{{ADMIN:'系统管理员',OPS:'运维',DEVELOPER:'开发',TESTER:'测试'}[user.role]??user.role}</small></div><button title="退出" aria-label="退出登录" onClick={()=>void logout()}>↪</button></div></header><div className="content"><Outlet/></div></main>
  </div>
}

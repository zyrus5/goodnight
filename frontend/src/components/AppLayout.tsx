...
export function AppLayout(){
  ...
  return <div className="app-shell">
    <aside className="sidebar"><div className="brand"><span className="brand-mark">G</span><div><strong>Goodnight</strong><small>Jenkins 编排平台</small></div></div>
      ...
      <div className="sidebar-user"><div className="avatar">{user.display_name.slice(0,1)}</div><div><strong>{user.display_name}</strong><small>{user.is_admin?'系统管理员':'普通用户'}</small></div><button title="退出" onClick={()=>void logout()}>↪</button></div>
    </aside>
